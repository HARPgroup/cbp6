...
anndat <- function (dfname, sd, ed, wateryear = FALSE) {
  if (wateryear) {
    yrstr <- "wateryear"
  } else {
    yrstr <- "year"
...
lseg <- 'N51003' # Shenandoah N51165, Middle James N51107, York: N51101, Albemarle 51003
fips <- substr(lseg,4,6)
county <- as.character(sqldf(
  paste0('select COUNTY_NAME from countyCd where STATE = \'51\' and COUNTY = \'', fips, '\'')
))
...
pbase_all_ann$excess <- pbase_all_ann$prcp - pbase_all_ann$evap
quantile(pbase_all_ann$excess)
mean(pbase_all_ann$excess)
eylm <- lm(evap ~ yindex, dat=as.data.frame(pbase_all_ann))
summary(eylm) 
bp<- ggplot(pbase_all_ann, aes(x=year, y=evap), ylim=c(0,40)) +
  geom_point(color="blue", fill=rgb(0.1,0.4,0.5,0.7))
bp + labs(
  title = paste('Evaporation in', county),
...
summary(pylm) 
bp<- ggplot(pbase_all_ann, aes(x=year, y=prcp), ylim=c(0.40)) +
  geom_point(color="blue", fill=rgb(0.1,0.4,0.5,0.7))
bp + labs(
