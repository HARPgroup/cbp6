...
container="C:\\Users\\emma\\Documents\\HARP\\Github\\cbp6\\code";
container2="C:\\Users\\emma\\Documents\\HARP\\Github";
...
---
title: "Appendix ##: River Segment Scenario 1: `r paste(riv.seg,': ',mod.scenario1," vs. Scenario 2: ", mod.scenario2)`"
header-includes:
  - \usepackage{titling}
...
```{r, include=FALSE}
#retrieve rest token
source(paste0(container2,"/auth.private")) #load rest username and password, contained in auth.private file

token <- rest_token(site, token, rest_uname, rest_pw);
options(timeout=120); # set timeout to twice default level to avoid abort due to high traffic

# Loading written gage description
description <- read_file(paste0(file.path(hydro_tools, "HARP-2018", "DEQ_Model_vs_USGS_Comparison", "gage_descriptions", site), ".txt"))

# Generating gage location maps
#gis_img <- fn_gage_and_seg_mapper(riv.seg, hydro_tools, token)
```
...
scale_colour_manual(values=c("black","red"))+
  ...
  annotate("text", x=xpos1, y=max, label= paste0('Difference:', '',difference1, '', '%'), size=3.5)+
  annotate("text", x=xpos2, y=max, label= paste0('Date Range: ', '', 
             min(storeplotdata1$Date),': ', max(storeplotdata1$Date)), size=3.5)+
  labs(y = "Flow (cfs)")
...
difference2plot <- ggplot(df, aes(x=Date)) + 
  ...
  scale_colour_manual(values=c("black","red"))+
  guides(colour = guide_legend(override.aes = list(size=5)))+
  annotate("text", x=xpos1, y=max, label= paste0('Difference:', '',difference2, '', '%'), size=3.75)+
  annotate("text", x=xpos2, y=max, label= paste0('Date Range: ', '', 
                                                 min(storeplotdata2$Date),': ', max(storeplotdata2$Date)), size=3.75)+
  labs(y = "Flow (cfs)")
...
difference3plot <- ggplot(df, aes(x=Date)) + 
  ...
  scale_colour_manual(values=c("black","red"))+
  guides(colour = guide_legend(override.aes = list(size=5)))+
  annotate("text", x=xpos1, y=max, label= paste0('Difference:', '',difference3, '', '%'), size=2)+
  annotate("text", x=xpos2, y=max, label= paste0('Date Range: ', '', 
                                                 min(storeplotdata3$Date),': ', max(storeplotdata3$Date)), size=2)+
  labs(y = "Flow (cfs)")
```
...
#description.cont <- paste0(" The average daily discharge change between scenario 1 and scenario 2 for the 20 year timespan was ", OUTPUT_MATRIXsaver[1,2] ,"%, with ", OUTPUT_MATRIXsaver[1,1], "% of its rolling three month time spans above 20% difference.")
#cat(description)
...
options(scipen=5, width = 1400, height = 950)
myplot <- ggplot(df, aes(x=Date)) + 
  geom_line(aes(y=Scenario1Flow, color=paste("Base 2018 Flow")), size=1)+
  geom_line(aes(y=Scenario2Flow, color=paste("Climate Change Flow")), size=0.5)+ 
  geom_line(aes(y=Scenario1Baseflow, color=paste("Base 2018 Baseflow")), size=0.5) +
  geom_line(aes(y=Scenario2Baseflow, color=paste("Climate Change Baseflow")), size=1)+
  fixtheyscale+ 
  theme_bw()+ 
  theme(legend.position="top", 
        legend.title=element_blank(),
...
resid <- (data$`Scenario 1 Flow` - data$`Scenario 2 Flow`)
...
options(scipen=5, width = 1400, height = 950)
myplot <- ggplot(df, aes(x=Date)) + 
  geom_point(aes(y=Residual, color="Base 2018"), size=1) +
  geom_line(aes(y=Zeroline, color="Climate Change"), size=0.8)+
