...
gage.to.seg <- read.csv('C:\\Users\\danie\\Documents\\HARP\\GitHub\\hydro-tools\\HARP-2018\\DEQ_Model_vs_USGS_Comparison_Northern\\data\\Gage_To_Segment_Northern.csv')
...
all.up <- c(shen.up, matt.up, pamu.up, rapp.up, appo.up, jaup.up, jami.up)

table <- as.data.frame(matrix(data = NA, nrow = length(which(all.up %in% gage.to.seg$segment)), ncol = 6))
colnames(table) <- c('River Segment', 'USGS Gage', 'Gage Start Date', 'Gage End Date', 'NSE', 'Rsq')
table$`River Segment` <- all.up[which(all.up %in% gage.to.seg$segment)]
for (i in 1:length(table$`River Segment`)) {
  table$`USGS Gage`[i] <- paste0("0", gage.to.seg$gage[which(gage.to.seg$segment == table$`River Segment`[i])])
}

rsq <- function (x, y) cor(x, y) ^ 2
library(hydroGOF)

gage.means <- rep(NA, length(table$`River Segment`))
seg.means <- rep(NA, length(table$`River Segment`))

gage.ALFs <- rep(NA, length(table$`River Segment`))
seg.ALFs <- rep(NA, length(table$`River Segment`))

gage.S10s <- rep(NA, length(table$`River Segment`))
seg.S10s <- rep(NA, length(table$`River Segment`))

gage.l30s <- rep(NA, length(table$`River Segment`))
seg.l30s <- rep(NA, length(table$`River Segment`))

gage.l90s <- rep(NA, length(table$`River Segment`))
seg.l90s <- rep(NA, length(table$`River Segment`))

library(dataRetrieval)
for (i in 1:length(table$`River Segment`)) {
  seg <- as.character(table$`River Segment`[i])
  gage <- as.character(table$`USGS Gage`[i])
  seg.area <- gage.to.seg$segDA[which(gage.to.seg$segment == seg)]
  gage.area <- gage.to.seg$gageDA[which(gage.to.seg$segment == seg)]
  
  gage.dat <- readNWISdv(gage, '00060', '1991-01-01', '2000-12-31')
  seg.dat <- read.csv(paste0('~/Precip_and_Temp_Mapper/Runit_Qout_data/', paste0('qout_', seg), '.csv'))
  seg.dat$data11.flow <- seg.dat$data11.flow * gage.area / seg.area
  
  data <- data.frame(cbind(gage.dat$X_00060_00003, seg.dat$data11.flow[which(as.Date(seg.dat$data11.date) %in% as.Date(gage.dat$Date))]))
  table$`Gage Start Date`[i] <- as.Date(gage.dat$Date[1])
  table$`Gage End Date`[i] <- as.Date(gage.dat$Date[length(gage.dat$Date)])
  table$NSE[i] <- round(NSE(data$X2, data$X1), 2)
  table$Rsq[i] <- round(rsq(data$X1, data$X2), 2)
  
  gage.namer <- paste0('gage_', gage)
  seg.namer <- paste0('seg_', seg)
  
  assign(gage.namer, gage.dat)
  assign(seg.namer, seg.dat)
  
  gage.dat2 <- data.frame(gage.dat$Date, gage.dat$X_00060_00003)
  seg.dat2 <- data.frame(gage.dat$Date, seg.dat$data11.flow[which(as.Date(seg.dat$data11.date) %in% as.Date(gage.dat$Date))])
  colnames(gage.dat2) <- c('date', 'flow')
  colnames(seg.dat2) <- c('date', 'flow')
  
  gage.means[i] <- mean(gage.dat2$flow)
  seg.means[i] <- mean(seg.dat2$flow)
  
  gage.ALFs[i] <- monthly_min(gage.dat2, num.month = 8)
  seg.ALFs[i] <- monthly_min(seg.dat2, num.month = 8)

  gage.S10s[i] <- sept_10_flow(gage.dat2)
  seg.S10s[i] <- sept_10_flow(seg.dat2)
  
  gage.l30s[i] <- num_day_min(gage.dat2, num.day = 30, min_or_med = "min")
  seg.l30s[i] <- num_day_min(seg.dat2, num.day = 30, min_or_med = "min")
  
  gage.l90s[i] <- num_day_min(gage.dat2, num.day = 90, min_or_med = "min")
  seg.l90s[i] <- num_day_min(seg.dat2, num.day = 90, min_or_med = "min")
}

kable(table,  booktabs = T,
      caption = "Model Performance Assessment between Historical Streamflow and VA Hydro Base Scenario",
      label = "modperformance",
  ...
  cat(., file = paste("modperformance",file_ext,sep=""))

shen.nse <- mean(table$NSE[1:13])
shen.rsq <- mean(table$Rsq[1:13])
matt.nse <- mean(table$NSE[14:15])
matt.rsq <- mean(table$Rsq[14:15])
pamu.nse <- mean(table$NSE[16:20])
pamu.rsq <- mean(table$Rsq[16:20])
rapp.nse <- mean(table$NSE[21:25])
rapp.rsq <- mean(table$Rsq[21:25])
jaup.nse <- mean(table$NSE[30:47])
jaup.rsq <- mean(table$Rsq[30:47])
jami.nse <- mean(table$NSE[48:63])
jami.rsq <- mean(table$Rsq[48:63])
appo.nse <- mean(table$NSE[26:29])
appo.rsq <- mean(table$Rsq[26:29])

means <- data.frame(gage.means, seg.means)
alfs <- data.frame(gage.ALFs, seg.ALFs)
sq10s <- data.frame(gage.S10s, seg.S10s)
l30s <- data.frame(gage.l30s, seg.l30s)
l90s <- data.frame(gage.l90s, seg.l90s)

means$perdiff <- 100*(means$seg.means-means$gage.means)/means$gage.means
alfs$perdiff <- 100*(alfs$seg.ALFs-alfs$gage.ALFs)/alfs$gage.ALFs
sq10s$perdiff <- 100*(sq10s$seg.S10s-sq10s$gage.S10s)/sq10s$gage.S10s
l30s$perdiff <- 100*(l30s$seg.l30s-l30s$gage.l30s)/l30s$gage.l30s
l90s$perdiff <- 100*(l90s$seg.l90s-l90s$gage.l90s)/l90s$gage.l90s
...
median(means$perdiff)
median(alfs$perdiff)
median(sq10s$perdiff)
median(l30s$perdiff)
median(l90s$perdiff)

gage.tab <- data.frame(table$`River Segment`, table$`USGS Gage`, round(gage.means, 1), 
                       round(gage.l30s, 1), round(gage.l90s, 1), 
                      round(gage.ALFs, 1), round(gage.S10s, 1))
names(gage.tab) <- c('River Segment', 'USGS Gage', 'Overall Mean Flow (cfs)', 
                     '30-Day Low Flow (cfs)', '90-Day Low Flow (cfs)', 
                     'August Low Flow (cfs)', 'September 10% Flow (cfs)')

seg.tab <- data.frame(table$`River Segment`, table$`USGS Gage`, round(seg.means, 1), 
                     round(seg.l30s, 1), round(seg.l90s, 1), round(seg.ALFs, 1), round(seg.S10s, 1))
names(seg.tab) <- c('River Segment', 'USGS Gage', 'Overall Mean Flow (cfs)', 
                     '30-Day Low Flow (cfs)', '90-Day Low Flow (cfs)', 
                     'August Low Flow (cfs)', 'September 10% Flow (cfs)')

diff.tab <- data.frame(table$`River Segment`, table$`USGS Gage`, round(means$perdiff, 2), 
                       round(l30s$perdiff, 2), round(l90s$perdiff, 2), round(alfs$perdiff, 2), round(sq10s$perdiff, 2))
names(diff.tab) <- c('River Segment', 'USGS Gage', 'Overall Mean Flow (%)', 
                     '30-Day Low Flow (%)', '90-Day Low Flow (%)', 
                     'August Low Flow (%)', 'September 10% Flow (%)')

kable(gage.tab,  booktabs = T,
      caption = "Streamflow Metrics of USGS Gage Data",
      label = "gage.tab",
      col.names = colnames(gage.tab)) %>%
  kable_styling(latex_options = latexoptions) %>%
  #column_spec(1, width = "6em") %>%
  #column_spec(2, width = "5em") %>%
  #column_spec(3, width = "5em") %>%
  #column_spec(4, width = "4em") %>%
  cat(., file = paste("gage.tab",file_ext,sep=""))

kable(seg.tab,  booktabs = T,
      caption = "Streamflow Metrics of VA Hydro River Segment Data",
      label = "seg.tab",
      col.names = colnames(seg.tab)) %>%
  kable_styling(latex_options = latexoptions) %>%
  #column_spec(1, width = "6em") %>%
  #column_spec(2, width = "5em") %>%
  #column_spec(3, width = "5em") %>%
  #column_spec(4, width = "4em") %>%
  cat(., file = paste("seg.tab",file_ext,sep=""))

kable(diff.tab,  booktabs = T,
      caption = "Percent Difference in Streamflow Metrics Between USGS Gage and VA Hydro River Segment Data",
      label = "diff.tab",
      col.names = colnames(diff.tab)) %>%
  kable_styling(latex_options = latexoptions) %>%
  #column_spec(1, width = "6em") %>%
  #column_spec(2, width = "5em") %>%
  #column_spec(3, width = "5em") %>%
  #column_spec(4, width = "4em") %>%
  cat(., file = paste("diff.tab",file_ext,sep=""))