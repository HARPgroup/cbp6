...
for (i in 1:loop){                                # run loop for an entire data series
  ...
  for (m in 1:12){
    ...
    avgmonth_scenario1 <- mean(month_time$`Scenario 1 Flow`)   # find average of gage flow for 3 months
    avgmonth_scenario2 <- mean(month_time$`Scenario 2 Flow`) # find average of model flow for 3 months
...
difference1 <- signif(HighestDifferences$Difference[1], digits=3)     #Create error variable to display on graph
...
difference2plot <- ggplot(df, aes(x=Date)) + 
  geom_line(aes(y=Scenario_1, color=name_Scenario1), size=1) +
  geom_line(aes(y=Scenario_2, color=name_Scenario2), size=1)+
  theme_bw()+ 
  theme(legend.position="top", 
        legend.title=element_blank(),
        legend.box = "horizontal", 
...
difference3plot <- ggplot(df, aes(x=Date)) + 
  geom_line(aes(y=Scenari, color=name_Scenario1), size=1) +
  geom_line(aes(y=Scenario_2, color=name_Scenario2), size=1)+
  theme_bw()+ 
  theme(legend.position="top", 
        legend.title=element_blank(),
...
low.year <- subset(all_data, all_data$year==low.year);
