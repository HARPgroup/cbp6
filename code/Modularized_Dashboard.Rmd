...
for (i in 1:length(cbp6.landunits)) {
  ...
  namer <- paste0('tab.', cbp6.landunits[i], '.iqr.by.lrseg.flow')
  tmp.tab <- tab.iqr.by.lrseg.flow(tmp.data)
  assign(namer, tmp.tab)
  ...
  namer <- paste0('tab.', cbp6.landunits[i], '.iqr.by.lrseg.land.use')
  tmp.tab <- tab.iqr.by.lrseg.land.use(tmp.data)
  assign(namer, tmp.tab)
}
...
fig10.runit.boxplot(riv.seg, run.id, token, site, start.date, end.date)
