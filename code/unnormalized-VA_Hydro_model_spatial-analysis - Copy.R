...
# INPUTS ------------------------------------------------------------------
unnormalized_spatial_analysis <- function(mod.scenario, github_link, site) {
  ...
  #LOAD STATE GEOMETRY
  #--------------------------------------------------------------------------------------------
  STATES <- read.table(file=paste(cbp6,'code',"GIS_LAYERS","STATES.tsv",sep="\\"), header=TRUE, sep="\t") #Load state geometries
