...
---
title: "River Segment: `r paste(riv.seg,'- Scenario : ',mod.scenario1,": VaHydro Run 116 vs. VAHydro Run 117 (fixed channel length)")`"
header-includes:
  - \usepackage{titling}
...
options(scipen=5, width = 1400, height = 950)
myplot <- ggplot(df, aes(x=Date)) + 
  geom_line(aes(y=Scenario1Flow, color=paste("VAHydro Scen. 1 Flow")), size=1)+
  geom_line(aes(y=Scenario2Flow, color=paste("VAHydro Scen. 2 Flow")), size=0.5)+ 
  geom_line(aes(y=Scenario1Baseflow, color=paste("VAHydro Scen. 1 Baseflow")), size=0.5) +
  geom_line(aes(y=Scenario2Baseflow, color=paste("VAHydro Scen. 2 Baseflow")), size=1)+
  fixtheyscale+ 
  theme_bw()+ 
  theme(legend.position="top", 
        legend.title=element_blank(),
