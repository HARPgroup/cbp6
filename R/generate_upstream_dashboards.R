...
generate_upstream_dashboards <- function(riv.seg, dat.source1, dat.source2, start.date, end.date, github_link,
                                         site, site.or.server, run.id1, run.id2, gage_number, mod.phase1,
                                         mod.scenario1, mod.phase2, mod.scenario2) {
  cbp6_link = paste0(github_link, "/cbp6/code");
  
  # CREATING OUTLET FOLDER AND FILE NAME
  if (dat.source1 == 'vahydro') {
    pt1 = paste0('run_', run.id1)
  ...
  setwd(cbp6_link)
  dir.create(paste0('/opt/model/p6/p6_gb604/out/dashboards/', output.folder.name))
  dash.output.dir <- paste0('/opt/model/p6/p6_gb604/out/dashboards/', output.folder.name)
  
  # Sourcing upstream functions
  ...
  token2 <- rest_token(site, token2, rest_uname, rest_pw);
  ...
  for(i in 1:length(all.upstream.segs)) {
    ...
    rmarkdown::render(paste0("Modularized_Dashboard_Server.Rmd"), "pdf_document", output_dir = dash.output.dir, output_file = paste0(riv.seg, ".pdf"), 
                      params = list(riv.seg = riv.seg, dat.source1 = dat.source1, dat.source2 = dat.source2,
                                    ...
                                    mod.scenario2 = mod.scenario2, token1 = token2))
  }
}